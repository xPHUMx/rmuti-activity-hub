    ...
    <div className="min-h-screen bg-gray-900 text-white">
                ...
                <tr key={a._id} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition cursor-pointer">
                  ...
                  <td className="p-3">
                    {registeredActivities.includes(a._id) ? (
                      ...
                      <button onClick={() => { setSelectedActivity(a); setShowPopup(true); }} className="bg-orange-600 hover:bg-blue-400 text-white px-4 py-2 rounded-xl">ลงทะเบียน</button>
                    ) : (
                      <span className="text-gray-400 italic">ปิด</span>
                    )}
                  </td>
      ...
      {showPopup && selectedActivity && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-2xl w-96">
            <h2 className="text-2xl font-bold mb-4 text-center text-blue-500">ยืนยันการลงทะเบียน</h2>
            <p className="mb-4 text-center text-black dark:text-white">
              คุณต้องการลงทะเบียนกิจกรรม <strong>{selectedActivity.title}</strong> ใช่ไหม?
            </p>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">ชั้นปี/กลุ่มเรียน</label>
              <input
                type="text"
                name="year"
                value={userInfo.year}
                onChange={handleTextChange}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-black focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
