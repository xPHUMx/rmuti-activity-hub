    ...
    if (isNewsRelated(userInput)) {
      ...
      if (isNewsSummary(userInput)) {
        if (!newsList.length) {
          ...
          setMessages((msgs) => [
            ...
            {
              sender: "bot",
              text: `นี่ข่าวอัปเดตล่าสุด ${newsMsgs.length} 👇`,
            },
            ...newsMsgs,
          ]);
        }
      ...
      if (found) {
        ...
        setMessages((msgs) => [
          ...
          {
            sender: "bot",
            text: "ยังไม่มีข่าวที่ตรงกับที่ถามมาเลยน้าา ลองถามใหม่อีกทีได้นะ",
          },
        ]);
      }
      setLoading(false);
...
`;
  ...
  const handleClearChat = () => {
    setMessages([
      {
        sender: "bot",
        text: "สวัสดี! ถามเรื่องกิจกรรม ข่าวสาร หรืออะไรก็ได้เลยน้า😊",
      },
    ]);
  };
